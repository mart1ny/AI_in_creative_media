...
def main() -> None:
    ...
    comparison = {
        ...
        "same_image_size": manifests[0]["image_size_observed"]
        == manifests[1]["image_size_observed"],
        "same_image_mode": manifests[0]["image_mode_observed"]
        == manifests[1]["image_mode_observed"],
        "interpretation": (
            "PASS: два запуска в одной зафиксированной среде побитово совпали."
            if same_hash
            else "FAIL: SHA-256 различаются. Для GPU это может быть "
            "недетерминизм CUDA, а не ошибка кода; фиксируем факт и диагностику."
        ),
    }
